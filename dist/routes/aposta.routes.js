...
import { betEventBus, emitBetEvent } from '../utils/betEvents.js';
...
router.get('/', authenticateToken, async (req, res) => {
    try {
        const userId = req.userId;
        const { esporte, status, tipster, dataInicio, dataFim, casa, bancaId, oddMin, oddMax } = req.query;
        const bancas = await prisma.bankroll.findMany({
...
router.put('/:id', authenticateToken, betUpdateRateLimiter, async (req, res) => {
    try {
        const { id } = req.params;
        ...
        const updateData = {};
        if (data.bancaId)
            updateData.bancaId = data.bancaId;
        if (data.esporte) {
            updateData.esporte = normalizarEsporteParaOpcao(data.esporte) || data.esporte;
        }
        if (data.evento)
            updateData.jogo = data.evento;
        if (data.torneio !== undefined)
            updateData.torneio = data.torneio;
        if (data.pais !== undefined)
            updateData.pais = data.pais;
        if (data.mercado)
            updateData.mercado = data.mercado;
        if (data.tipoAposta)
            updateData.tipoAposta = data.tipoAposta;
        if (data.valorApostado)
            updateData.valorApostado = data.valorApostado;
        if (data.odd)
            updateData.odd = data.odd;
        if (data.bonus !== undefined)
            updateData.bonus = data.bonus;
        const updatedDate = resolveEventoDate(data);
        if (updatedDate)
            updateData.dataJogo = updatedDate;
        if (data.tipster !== undefined)
            updateData.tipster = data.tipster;
        if (data.status)
            updateData.status = data.status;
        if (data.casaDeAposta)
            updateData.casaDeAposta = data.casaDeAposta;
...
//# sourceMappingURL=aposta.routes.js.map