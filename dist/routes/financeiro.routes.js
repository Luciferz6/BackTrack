...
router.post('/transacao', authenticateToken, async (req, res) => {
    try {
...
router.put('/transacao/:id', authenticateToken, async (req, res) => {
    try {
        ...
        if (body.tipo)
            data.tipo = body.tipo;
        if (body.casaDeAposta)
            data.casaDeAposta = body.casaDeAposta;
        if (typeof body.valor === 'number')
            data.valor = body.valor;
        if (body.dataTransacao)
            data.dataTransacao = new Date(body.dataTransacao);
...
router.delete('/transacao/:id', authenticateToken, async (req, res) => {
    try {
        const params = deleteSchema.parse({ id: req.params.id });
        const userId = req.userId;
        const transacao = await prisma.financialTransaction.findUnique({
...
router.get('/transacoes', authenticateToken, async (req, res) => {
    try {
        ...
        if (casa) {
            where.casaDeAposta = { contains: casa };
        }
        if (bancaId) {
        ...
        if (dataInicio || dataFim) {
            where.dataTransacao = {};
            if (dataInicio) {
                where.dataTransacao.gte = new Date(dataInicio);
            }
            if (dataFim) {
                where.dataTransacao.lte = new Date(dataFim);
            }
        }
...
router.get('/saldo-geral', authenticateToken, async (req, res) => {
    try {
        const userId = req.userId;
