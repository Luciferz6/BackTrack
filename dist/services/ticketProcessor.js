...
const buildTextPrompt = (text) => `${TICKET_PROMPT}
...
const normalizeExtractedData = (extractedData) => ({
    ...
    valorApostado: typeof extractedData.valorApostado === 'number'
        ? extractedData.valorApostado
        : parseFloat(String(extractedData.valorApostado || '0')
            .replace(/[^\d.,]/g, '')
            .replace(',', '.')) || 0,
    odd: typeof extractedData.odd === 'number'
        ? extractedData.odd
        : parseFloat(String(extractedData.odd || '0')
            .replace(/[^\d.,]/g, '')
            .replace(',', '.')) || 0,
    dataJogo: extractedData.dataJogo ? String(extractedData.dataJogo).split('T')[0] : '',
    status: extractedData.status || 'Pendente'
});
const buildImagePrompt = (ocrText) => {
    if (ocrText?.trim()) {
...
const processWithGemini = async (base64Image, mimeType, ocrText) => {
    if (!genAI) {
...
const processWithDeepseek = async (base64Image, mimeType, ocrText) => {
    ...
    if (typeof messageContent === 'string') {
        ...
        content_text = messageContent.map((part) => part?.text || part?.content || '').join('\n').trim();
    }
...
export const processTicket = async ({ base64Image, mimeType, ocrText }) => {
    const attempts = [];
...
//# sourceMappingURL=ticketProcessor.js.map